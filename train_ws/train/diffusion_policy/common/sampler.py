...
class SequenceSampler:
    def __init__(self,
        ...
        # optional: use gripper_width to mark "before first grasp"
        self.repeat_frame_prob = repeat_frame_prob
        has_gripper_width = ('robot0_gripper_width' in replay_buffer.keys())
        if has_gripper_width:
            gripper_width = replay_buffer['robot0_gripper_width'][:, 0]
            gripper_width_threshold = 0.08
        else:
            gripper_width = None
            gripper_width_threshold = None
        ...
        for i in range(len(episode_ends)):
            ...
            for current_idx in range(start_idx, end_idx):
                ...
                if has_gripper_width:
                    ...
                    # hand_umi (no gripper width): disable "before first grasp" augmentation
                    before_first_grasp = False
        ...
        #     self.replay_buffer['action'] = np.concatenate(actions, axis=-1)
            ...
            # construct action per-robot:
            # - if hand nodes exist:  [eef_pos(3), eef_rot_axis_angle(3), hand_nodes(75)]  -> 81
            # - elif hand angles exist:[eef_pos(3), eef_rot_axis_angle(3), hand_angles(6)] -> 12
            # - elif gripper exists:  [eef_pos(3), eef_rot_axis_angle(3), gripper_width(1)] -> 7
            actions = []

            use_hand_nodes = any((f'robot{r}_hand_nodes' in self.replay_buffer) for r in range(self.num_robot))
            use_hand_angles = any((f'robot{r}_hand_angles' in self.replay_buffer) for r in range(self.num_robot))
            use_gripper = any((f'robot{r}_gripper_width' in self.replay_buffer) for r in range(self.num_robot))

            if use_hand_nodes:
                for robot_idx in range(self.num_robot):
                    pos_key = f'robot{robot_idx}_eef_pos'
                    rot_key = f'robot{robot_idx}_eef_rot_axis_angle'
                    hand_key = f'robot{robot_idx}_hand_nodes'

                    missing = [k for k in [pos_key, rot_key, hand_key] if k not in self.replay_buffer]
                    if len(missing) > 0:
                        raise KeyError(f"Missing keys for hand_nodes action: {missing}")

                    actions.append(self.replay_buffer[pos_key])
                    actions.append(self.replay_buffer[rot_key])
                    actions.append(self.replay_buffer[hand_key])

            elif use_hand_angles:
                for robot_idx in range(self.num_robot):
                    pos_key = f'robot{robot_idx}_eef_pos'
                    rot_key = f'robot{robot_idx}_eef_rot_axis_angle'
                    hand_key = f'robot{robot_idx}_hand_angles'

                    missing = [k for k in [pos_key, rot_key, hand_key] if k not in self.replay_buffer]
                    if len(missing) > 0:
                        raise KeyError(f"Missing keys for hand_angles action: {missing}")
                ...
                for robot_idx in range(self.num_robot):
                    ...
                    missing = [k for k in [pos_key, rot_key, grip_key] if k not in self.replay_buffer]
                    if len(missing) > 0:
                        raise KeyError(f"Missing keys for gripper action: {missing}")

                    actions.append(self.replay_buffer[pos_key])
                    actions.append(self.replay_buffer[rot_key])
                    actions.append(self.replay_buffer[grip_key])

            else:
                raise KeyError(
                    "Cannot construct action: neither hand_nodes nor hand_angles nor gripper_width exists in replay_buffer."
                )
