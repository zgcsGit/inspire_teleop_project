...
class UmiDataset(BaseDataset):
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        for i in range(self.num_robot):
            ...
            if dim_a == 10:
                ...
                action_normalizers.append(
                    get_range_normalizer_from_stat(array_to_stats(data_cache['action'][..., base: base + 3]))
                )
                action_normalizers.append(
                    get_identity_normalizer_from_stat(array_to_stats(data_cache['action'][..., base + 3: base + 9]))
                )
                action_normalizers.append(
                    get_range_normalizer_from_stat(array_to_stats(data_cache['action'][..., base + 9: base + 10]))
                )
                
            elif dim_a == 12:
                # pos(3), rot6d(6), active hand_angles(3)
                action_normalizers.append(
                    get_range_normalizer_from_stat(array_to_stats(data_cache['action'][..., base: base + 3]))
                )
                action_normalizers.append(
                ...
                action_normalizers.append(
                    get_range_normalizer_from_stat(array_to_stats(data_cache['action'][..., base + 9: base + 12]))
                )

            elif dim_a == 15:
                # pos(3), rot6d(6), hand_angles(6)
                action_normalizers.append(
                    get_range_normalizer_from_stat(array_to_stats(data_cache['action'][..., base: base + 3]))
                )
                action_normalizers.append(
                ...
                action_normalizers.append(
                    get_range_normalizer_from_stat(array_to_stats(data_cache['action'][..., base + 9: base + 15]))
                )

            elif dim_a == 84:
                # pos(3), rot6d(6), hand_nodes(75)
                action_normalizers.append(
                    get_range_normalizer_from_stat(array_to_stats(data_cache['action'][..., base: base + 3]))
                )
                action_normalizers.append(
                ...
                action_normalizers.append(
                    get_range_normalizer_from_stat(array_to_stats(data_cache['action'][..., base + 9: base + 84]))
                )
            else:
                raise RuntimeError(f"Unsupported per-robot action dim: {dim_a}. Expected 10, 12, 15, or 84.")
        ...
        for key in self.lowdim_keys:
            ...
            if key.endswith('pos') or 'pos_wrt' in key:
                ...
                this_normalizer = get_range_normalizer_from_stat(stat)
            elif key.endswith('hand_nodes'):
                this_normalizer = get_range_normalizer_from_stat(stat)
            elif key.endswith('hand_angles'):
                this_normalizer = get_range_normalizer_from_stat(stat)
            elif '_touch_' in key:
                this_normalizer = get_range_normalizer_from_stat(stat)
            else:
                raise RuntimeError(f'unsupported key for normalizer: {key}')
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        actions = []
        # raw action dimension per robot (from SequenceSampler)
        assert data['action'].shape[-1] % self.num_robot == 0
        raw_dim_a = data['action'].shape[-1] // self.num_robot
        if raw_dim_a not in [7, 9, 12, 81]:
            raise RuntimeError(f"Unsupported raw per-robot action dim: {raw_dim_a}. Expected 7, 9, 12, or 81.")

        for robot_id in range(self.num_robot):
            # current obs pose (rotvec3 at this point)
            pose_mat = pose_to_mat(np.concatenate([
                obs_dict[f'robot{robot_id}_eef_pos'],
                obs_dict[f'robot{robot_id}_eef_rot_axis_angle']
            ], axis=-1))

            base = robot_id * raw_dim_a

            # first 6 dims are always pose: pos3 + rotvec3
            action_pose6 = data['action'][..., base: base + 6]
            action_mat = pose_to_mat(action_pose6)

            # convert pose representation (relative/abs/delta) using last obs pose as base
            obs_pose_mat = convert_pose_mat_rep(
                pose_mat,
                base_pose_mat=pose_mat[-1],
                pose_rep=self.obs_pose_repr,
            ...
            # convert to pos(3) + rot6d(6) => pose10d without gripper/hand
            obs_pose = mat_to_pose10d(obs_pose_mat)        # (T,9)
            action_pose = mat_to_pose10d(action_pose_mat)  # (T,9)

            # tail part
            if raw_dim_a == 7:
                # gripper scalar (1)
                tail = data['action'][..., base + 6: base + 7]  # (T,1)
                actions.append(np.concatenate([action_pose, tail], axis=-1))  # (T,10)
            elif raw_dim_a == 9:
                # active hand angles (3)
                tail = data['action'][..., base + 6: base + 9]  # (T,3)
                actions.append(np.concatenate([action_pose, tail], axis=-1))  # (T,12)
            elif raw_dim_a == 12:
                # hand angles (6)
                tail = data['action'][..., base + 6: base + 12]  # (T,6)
                actions.append(np.concatenate([action_pose, tail], axis=-1))  # (T,15)
            else:
                # hand nodes (75)
                tail = data['action'][..., base + 6: base + 81]  # (T,75)
                actions.append(np.concatenate([action_pose, tail], axis=-1))  # (T,84)

            # overwrite obs with rot6d representation for policy input
            obs_dict[f'robot{robot_id}_eef_pos'] = obs_pose[:, :3]
            obs_dict[f'robot{robot_id}_eef_rot_axis_angle'] = obs_pose[:, 3:]  # (T,6)
