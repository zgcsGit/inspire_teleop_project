...
namespace {
std::string default_robot_ip() {
    const char* env_ip = std::getenv("RIGHT_FRANKA_ROBOT_IP");
    return env_ip != nullptr ? std::string(env_ip) : "12.1.1.6";
}
}  // namespace

FrankaNode::FrankaNode(const std::string& node_name)
: Node(node_name) {
    const std::string robot_ip = this->declare_parameter<std::string>(
        "robot_ip",
        default_robot_ip()
    );
    RCLCPP_INFO(this->get_logger(), "Connecting to right Franka at %s", robot_ip.c_str());
    ...
    const std::string actual_ee_pose_topic = this->declare_parameter<std::string>(
    ...
    control_shutdown = [this]() {
        if (robot_) {
            robot_->stop();
        }
        std::cout << "Stopped RIGHT Franka Control" << std::endl;
    };
    ...
    control_thread_ = std::thread([this]() {
        try {
            ...
            // Move to a known right-arm posture before accepting teleop pose targets.
            std::cout << "MOVING RIGHT ROBOT TO HOME POSITION." << std::endl;
            ...
            impedance_control_callback =
                [&, this](
                    ...
                    // Rate-limit target changes so noisy hand tracking cannot create jumps.
            ...
            std::cout << "WARNING: Starting RIGHT Cartesian impedance control."
                      << std::endl;
...
FrankaNode::~FrankaNode() {
    ...
    std::cout << "Shutting down RIGHT Franka control node" << std::endl;
}
...
void FrankaNode::desiredPoseCallback(
    ...
    // Fixed calibration between the hand-derived target frame and the Franka EE frame.
    // Keep this matrix here until it is moved into a calibration/config file.
    Eigen::Matrix4d T_diff_inv;
    T_diff_inv <<
        0.978148,  0.0,       0.207912, -0.009022,
        0.0,       1.0,       0.0,       0.000442,
    -0.207912,  0.0,       0.978148,  0.002423,
        0.0,       0.0,       0.0,       1.0;
